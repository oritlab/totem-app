import { FilterBarProps, GridColumns, SORT_OPTION_LABELS, SortOption } from "../types";
...
function ClearIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
      ...
      <path d="M4.8 4.8l4.4 4.4M9.2 4.8l-4.4 4.4" stroke="white" strokeWidth="1.2" strokeLinecap="round" />
    </svg>
  );
}
...
function ColumnsIcon(props: { count: GridColumns }) {
  const { count } = props;
...
type SortMenuProps = Pick<
  FilterBarProps,
  "sortOption" | "onSortChange" | "isSortOpen" | "sortRef" | "onToggleSort" | "onCloseSort"
>;

function SortMenu(props: SortMenuProps) {
  const { sortOption, onSortChange, isSortOpen, sortRef, onToggleSort, onCloseSort } = props;
  ...
  return (
    <div className="flex items-center justify-between gap-4 border-b border-zinc-200 px-6 py-4 text-sm text-zinc-700">
      <div className="flex items-center gap-3">
        ...
        <span onClick={onOpenFilter} className="cursor-pointer">
          <FilterIcon />
        </span>
